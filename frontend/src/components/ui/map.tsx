...
/*
 * Vendored from mapcn (https://mapcn.dev) via `shadcn add @mapcn/map`.
 * Local edits: self-hosted worker URL, loader class hook. Its internal
 * ref-during-render patterns and hook exports trip this repo's stricter
 * lint rules; treat as third-party code and don't refactor casually.
 */
...
if (typeof window !== "undefined" && !MapLibreGL.getWorkerUrl()) {
  // Self-hosted worker (copied into public/) so map rendering doesn't depend
  // on unpkg reachability. Keep the two files side by side when upgrading.
...
function DefaultLoader() {
  return (
    <div className="mapcn-loader bg-background/50 absolute inset-0 z-10 flex items-center justify-center backdrop-blur-xs">
